...
function openMgrDash() {
  ...
  const el = $("v-mgr");
  if (!el) return;
  el.innerHTML = "<div class='dash-header'><div><div class='dash-name'>Manager Dashboard</div><div class='dash-sub'>Tap+ AI</div></div><button class='dash-exit' onclick='showHub()'>← Exit</button></div><div class='dash-tabs' id='mgr-tabs'><button class='dash-tab ai active' onclick='mgrTab(\"ai\",this)'><span class='ai-mini-dot'></span> AI Insights</button><button class='dash-tab' onclick='mgrTab(\"team\",this)'>Team</button><button class='dash-tab ai' onclick='mgrTab(\"estimator\",this)'><span class='ai-mini-dot'></span> Estimator</button><button class='dash-tab' onclick='mgrTab(\"staff\",this)'>Staff</button></div><div class='dash-body' id='mgr-body'></div>";
...
function renderAIInsightsTab(body, activeStaff, sd, allFb) {
  ...
  var subLabels = { summary:"📋 Summary", coaching:"💬 Coaching", feedback:"🔍 Feedback", export:"📄 Export" };

  body.innerHTML =
    "<div id='ai-sub-tabs' style='display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap'>" +
      subBtns.map(function(s,i) {
        return "<button data-sub='" + s + "' onclick='aiSubTab(this.dataset.sub)' style='background:" + (i===0?"#a78bfa":"#15171f") + ";color:" + (i===0?"#07080c":"rgba(238,240,248,.5)") + ";border:1px solid " + (i===0?"#a78bfa":"rgba(255,255,255,.08)") + ";border-radius:9px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer'>" + subLabels[s] + "</button>";
      }).join("") +
    "</div>" +
  ...
  window.aiSubTab = function(sub) {
    ...
    if (sub === "summary") {
      ...
      el.innerHTML = "<div class='ai-card'><div class='ai-card-head'><div class='ai-card-ico'>🧠</div><div><div class='ai-card-title'>Weekly Performance Summary</div><div class='ai-card-sub'>" + ast.length + " active staff · " + new Date().toLocaleDateString([],{month:"short",day:"numeric"}) + "</div></div></div><div id='ai-summary' data-aiblock='1' data-prompt='" + encodeURIComponent(p) + "' data-msg='Generating summary…'></div></div>";
      ...
      el.innerHTML =
        ...
        ast.map(function(s) {
          return "<div class='pill" + (s.id===(first&&first.id)?" active":"") + "' onclick='selectCoachStaff(\"" + s.id + "\",this)'>" +
      ...
      var allFbItems = ast.flatMap(function(s) {
        ...
        return st.negFb.map(function(t) { return Object.assign({},t,{sName:s.name,sColor:s.color}); });
      }).sort(function(a,b) { return b.ts-a.ts; });
      var p2 = "Analyze private customer feedback:\n" + (fb2||"No feedback yet.") + "\nGive: 1) Overall sentiment 2) 2-3 recurring patterns 3) Operational issues 4) Urgent flags 5) Positive signals. Under 240 words.";
      el.innerHTML = "<div class='ai-card'><div class='ai-card-head'><div class='ai-card-ico'>🔍</div><div><div class='ai-card-title'>Sentiment &amp; Pattern Analysis</div><div class='ai-card-sub'>All private customer feedback</div></div></div><div id='ai-fb'></div></div>" +
        (allFbItems.length
          ? "<div class='sec-lbl' style='margin-top:4px'>Raw Feedback (" + allFbItems.length + ")</div>" + allFbItems.map(function(f) {
              var sv = f.rating>=4?{bg:"rgba(0,229,160,.08)",c:"#00e5a0",l:"positive"}:f.rating===3?{bg:"rgba(255,209,102,.08)",c:"#ffd166",l:"neutral"}:{bg:"rgba(255,68,85,.08)",c:"#ff4455",l:"negative"};
              return "<div class='plain-card'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px'><div style='font-weight:700;font-size:13px;color:" + f.sColor + "'>" + esc(f.sName) + "</div><div style='display:flex;align-items:center;gap:7px'><span style='background:" + sv.bg + ";color:" + sv.c + ";border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700'>" + sv.l + "</span><span style='font-size:11px;color:rgba(238,240,248,.38)'>" + fmt(f.ts) + "</span></div></div><div style='font-size:13px;margin-bottom:4px'>" + "⭐".repeat(f.rating) + "☆".repeat(5-f.rating) + "</div><div style='font-size:13px;color:rgba(238,240,248,.65);font-style:italic'>\"" + esc(f.feedback) + "\"</div></div>";
            }).join("")
          : "<div style='color:#00e5a0;font-size:13px;font-weight:500;margin-top:4px'>No feedback collected yet.</div>"
        );
      renderAIBlock("ai-fb", p2, "mgr_fb", "Analyzing feedback…");

    } else if (sub === "export") {
      var p3 = "Professional weekly report. DATE: " + new Date().toLocaleDateString([],{weekday:"long",year:"numeric",month:"long",day:"numeric"}) + "\nTEAM:\n" + sd2 + "\nFEEDBACK:\n" + (fb2||"None") + "\nSections: ## Executive Summary / ## Individual Staff Performance / ## Customer Sentiment / ## Key Recommendations / ## Goals for Next Week.";
      el.innerHTML = "<div class='ai-card'><div class='ai-card-head'><div class='ai-card-ico'>📄</div><div><div class='ai-card-title'>Full AI Performance Report</div><div class='ai-card-sub'>Ready to copy or print</div></div></div><button class='btn btn-ghost btn-sm' style='margin-bottom:6px' onclick='window.print()'>🖨 Print / Save PDF</button><div id='ai-report'></div></div>";
  ...
  body.innerHTML =
    "<div id='perf-sub-tabs' style='display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap'>" +
      subBtns.map(function(s) {
        ...
        return "<button data-ps='" + s + "' onclick='perfSubTab(this.dataset.ps)' style='background:" + (active?"#00e5a0":"#15171f") + ";color:" + (active?"#07080c":"rgba(238,240,248,.5)") + ";border:1px solid " + (active?"#00e5a0":"rgba(255,255,255,.08)") + ";border-radius:9px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer'>" + subLabels[s] + "</button>";
      }).join("") +
    "</div>" +
    ...
    rows.map(function(row, i) {
      ...
      return "<div class='lb-item " + (i<3?"r"+(i+1):"") + "' style='flex-direction:column;align-items:stretch;gap:10px'>" +
        ...
        "<div style='display:flex;align-items:center;gap:12px'>" +
          ...
          "<div style='flex:1'>" +
            "<div style='display:flex;align-items:center;gap:7px;margin-bottom:2px'>" +
              ...
              "<span style='font-size:10px;font-weight:700;color:" + pLabel.c + ";background:" + pLabel.c + "18;border-radius:5px;padding:1px 7px'>" + pLabel.l + "</span>" +
            "</div>" +
        ...
        "<div style='display:flex;align-items:center;gap:9px'>" +
          "<div style='font-size:11px;color:" + s.color + ";letter-spacing:1px;font-family:monospace;flex:1'>" + emojiBar + "</div>" +
          "<div style='font-size:10px;color:rgba(238,240,248,.35);font-weight:600;flex-shrink:0'>" + barPct + "%</div>" +
        "</div>" +
        // Actual progress bar
        "<div style='height:4px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden'>" +
          "<div style='height:100%;width:" + barPct + "%;background:linear-gradient(90deg," + s.color + "," + pLabel.c + ");border-radius:2px;transition:width .5s'></div>" +
        "</div>" +
      "</div>";
    }).join("") +
